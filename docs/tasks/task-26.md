# Task 26: Operational Runbooks

**Status:** todo
**Depends on:** 16, 20, 21, 22
...
- Check circuit breaker state (task 22 metrics)
- Remediation: fix upstream, or remove from config and reload if permanently gone
...
- Remediation: restore Redis, or temporarily switch to local rate limiting with config reload if global enforcement can be relaxed

### TLS

**`tls-certificate-expiry.md`** — TLS certificates approaching expiration.
- Check cert expiry dates
- Remediation: rotate certs, reload config (SIGHUP)
...
Add a "Troubleshooting" section to `docs/horizontal-scaling.md` (task 20) and the README linking to the runbooks directory.
