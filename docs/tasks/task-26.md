...
**Note:** Most production deployments terminate TLS at the load balancer (ALB, nginx, Caddy, etc.), so Stile typically receives plaintext on a private network. This task is for the single-binary / no-LB case.
...
When this task is implemented, update the README's "TLS / Transport Security" section to remove the note that Stile does not yet support native TLS, and document the new config options.
